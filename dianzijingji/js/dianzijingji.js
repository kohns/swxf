...
window.onload = function() {
    var box=document.getElementById("tainer");
    var imgs=box.getElementsByTagName("img");
    var imgWidth=imgs[0].offsetWidth;
    var mywidth=224;
    box.style.width=imgWidth + (imgs.length-1)*mywidth + "px";
    function setImagePos(){
        for(var i=1;i<imgs.length;i++){
            imgs[i].style.left=imgWidth+ (i-1)*mywidth + "px";
        }
    }
    setImagePos();
    var tx=imgWidth-mywidth;
    for(var i=0;i<imgs.length;i++){
        (function(i){
            imgs[i].onmouseover=function(){
                setImagePos();
                for(var j=1;j<=i;j++){
                    imgs[j].style.left=parseInt(imgs[j].style.left) - tx +"px";
                }
            }
        })(i);
    }
